...
import { createProgram, formatDiagnostic, NodeHost, } from "@cadl-lang/compiler";
import { getRestOperationDefinition } from "./rest.js";
import { writeFile } from "./write.js";
const SCHEMA_DIR = path.join(path.dirname(url.fileURLToPath(import.meta.url)), "..", "schemas");
export async function use(args) {
    ...
    const [operation] = wantedOperations;
    if (wantedOperations.length > 1) {
        console.warn("Multiple operations not yet supported, using only the first one:", operation.name);
    }
    const definition = getRestOperationDefinition(program, operation);
    process.stdout.write(writeFile(program, definition));
    return;
    function select(t, selector) {
    ...
    function getNamespaceChild(t, s) {
