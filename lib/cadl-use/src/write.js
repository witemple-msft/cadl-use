import { getIntrinsicModelName, isIntrinsic, } from "@cadl-lang/compiler";
...
function pascalCasify(s) {
    return s.split(/[-_]/).map(capitalize).join("");
}
export function writeFile(program, details) {
    const method = details.verb?.toUpperCase() ?? "GET";
    const requiredParams = details.parameters.parameters.filter((p) => !p.param.optional);
    const optionalParams = details.parameters.parameters.filter((p) => p.param.optional);
    const queryParams = details.parameters.parameters.filter((p) => p.type === "query");
    ...
    let cachedInterfaceDeclarations = new Map();
    let body = "";
    if (details.parameters.body) {
        const bodyModel = details.parameters.body;
        const bodyParamName = camelCasify(bodyModel.name);
        additionalPositionalParams += `,
    ${bodyParamName}: ${convertToTypeScript(bodyModel.type, pascalCasify(details.operation.name) + "RequestBody")}`;
        body = `,
...
`;
    ...
    function convertToTypeScript(t, preferredAlternativeName) {
        if (isIntrinsic(program, t)) {
            ...
            return ({
                ...
                (() => {
                    throw new Error("No TypeScript type for CADL intrinsic:" + t.toString());
                })());
        }
        switch (t.kind) {
            case "String":
    ...
    function addInterface(tOriginal, preferredAlternativeName) {
        ...
        const extendsClause = t.baseModel !== undefined
            ? ` extends ${convertToTypeScript(t.baseModel)}`
            : "";
        const fields = [...t.properties.values()].map((p) => `\n    ${p.name}${p.optional ? "?" : ""}: ${convertToTypeScript(p.type)}`);
